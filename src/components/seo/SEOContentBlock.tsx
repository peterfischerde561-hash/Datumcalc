        ...
        <Card as="section">
            ...
            <div className="space-y-5 text-ink-2 leading-relaxed text-lg">
                {content.paragraphs.map((p, i) => {
                    ...
                    const renderText = (t: string) => {
                        ...
                        return parts.map((part, index) => {
                            if (part.startsWith('**') && part.endsWith('**')) {
                                return <strong key={index} className="text-ink font-bold">{part.slice(2, -2)}</strong>;
                            }
                            return part;
                        });
                    };
                    ...
                    if (isH3) {
                        return <h3 key={i} className="text-xl font-bold text-ink mt-8 mb-4">{renderText(text)}</h3>;
                    }
                    return (
                        <p key={i}>
                            {renderText(text)}
            ...
            {content.weekdayTable && (
                <div className="mt-8 pt-8 border-t border-line">
                    <h3 className="text-xl font-semibold mb-4 text-ink">{content.weekdayTable.heading}</h3>
                            ...
                            <tbody className="text-ink-2 divide-y divide-line">
                                {content.weekdayTable.rows.map((row) => (
                                    <tr key={row.year} className="hover:bg-surface">
                                        <td className="py-3 px-4 font-medium text-ink">{row.year}</td>
            ...
            {content.useCases.length > 0 && (
                <div className="mt-8 pt-8 border-t border-line">
                    <h3 className="text-xl font-semibold mb-4 text-ink">{isDe ? 'Typische Anwendungsfälle' : 'Typical use cases'}</h3>
                    <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-ink-2">
